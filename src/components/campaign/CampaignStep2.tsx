...
export default function CampaignStep2({ onNext, onPrev, onSaveDraft, isEdit, submitTrigger = 0 }: CampaignStep2Props) {
    ...
    // 본문/사진 가이드 자동 설정: 카테고리별 과잉 맞춤 문구 대신 범용 문구만 기본 입력
    useEffect(() => {
        ...
        if (formData.includeNaver && (!formData.blogContentGuide || formData.blogContentGuide.trim() === '')) {
            const blogGuide = `[${productName}] 블로그 본문 작성 가이드\n- 제공받은 상품/서비스/체험의 첫인상과 사용 과정을 자연스럽게 작성해 주세요.\n- 제품 패키지, 구성품, 실제 사용 모습 또는 체험 장면을 사진과 함께 소개해 주세요.\n- 맛, 향, 사용감, 편의성, 만족도 등 직접 경험한 내용을 솔직하게 작성해 주세요.\n- 어떤 상황이나 사람에게 추천할 수 있는지 구체적으로 정리해 주세요.`;
        ...
        if (formData.includeInstagram && (!formData.instagramPhotoGuide || formData.instagramPhotoGuide.trim() === '')) {
            const instaGuide = `[${productName}] 인스타그램 업로드 가이드\n- 상품/서비스/체험의 특징이 잘 보이는 대표 사진 또는 영상을 포함해 주세요.\n- 실제 사용하거나 체험하는 장면을 자연스럽게 담아 주세요.\n- 좋았던 점과 추천 포인트를 짧고 명확하게 작성해 주세요.`;
            store.setField('instagramPhotoGuide', instaGuide);
        }
    }, [formData.productName, formData.campaignTitle, formData.includeNaver, formData.includeInstagram]);
