...
0) 작업 시작 필수 점검
- 사용자가 `작업시작하자`, `작업 다시하자`, `작업 시작하자` 등 작업 개시 의도를 밝히면 즉시 아래 상태를 확인한다.
- 필수 확인: `AGENTS.md` 존재/내용, 현재 브랜치와 작업트리 상태, 로컬 HEAD와 `origin/main` 최신 SHA 일치 여부, Vercel `daonview` 프로젝트 최신 production 배포 상태, 최근 24시간 production error/fatal 로그, 미해결 Vercel Toolbar 코멘트.
- `git ls-remote`처럼 로컬 파일을 바꾸지 않는 확인은 바로 수행한다.
- `git pull`, 파일 수정, DB 변경, 배포처럼 write 가능성이 있는 작업은 현재 상태를 보고한 뒤 사용자 명시 승인(`진행해`, `수정해`, `실행해`, `배포해`)을 받고 수행한다.
- 점검 결과는 작업 전 요약으로 보고한다.
...
캠페인 운영 테이블: 관리자/광고주 캠페인 목록과 운영 액션은 반드시 공통 컴포넌트 구조(`CampaignDataTable` + 역할별 래퍼)를 유지한다. 동일 기능을 관리자용/광고주용으로 분리 구현하거나 복붙하지 않는다.
...
### 검증 및 토큰 최적화
- 검증은 변경 위험과 범위에 비례해 수행하며, 모든 작업마다 전체 타입 검사와 프로덕션 빌드를 반복하지 않는다.
- 작업 중에는 수정 파일 중심의 ESLint, 관련 테스트, 화면 확인 등 가장 작은 유효 검증을 우선한다.
- 단순 문구·스타일·문서 수정은 타입/API 동작에 영향을 주지 않으면 전체 타입 검사와 빌드를 생략할 수 있다.
- 전체 TypeScript 검사는 타입, 데이터 흐름, 공용 인터페이스 또는 API 계약을 변경했을 때 실행한다.
- 프로덕션 빌드는 배포·마감 직전, 의존성·빌드 설정·라우팅·SSR 경계를 변경했을 때, 또는 사용자가 명시적으로 요청했을 때 1회 실행한다.
- 같은 코드 상태에서 이미 성공한 검사는 반복하지 않으며, 이후 변경이 해당 검사 결과에 영향을 줄 때만 다시 실행한다.
- 명령 출력은 성공 여부와 핵심 오류만 요약하고, 긴 정상 로그와 이미 알려진 기존 오류의 반복 출력은 피한다.
- 토큰 절약을 이유로 배포 전 필수 검증이나 데이터·인증·결제 등 고위험 변경 검증을 생략하지 않는다.
